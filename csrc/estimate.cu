...
void estimate(TensorView q, TensorView k, TensorView out) {
  CHECK_CONTIGUOUS(k)
  CHECK_CONTIGUOUS(out)
  CHECK_DIM(3, k);
  CHECK_DIM(2, out);

  unsigned int num_qo_heads, num_kv_heads, head_dim, seq_len;
  ...
  TVM_FFI_ICHECK_EQ(out.size(1), seq_len);
  ...
  bool success = DISPATCH_DLPACK_DTYPE_TO_CTYPE(q.dtype(), c_type, [&] {
    DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
      cudaError_t status = Estimate<HEAD_DIM, c_type>(
          ...
          static_cast<c_type*>(out.data_ptr()), num_qo_heads, num_kv_heads, seq_len, stream);
