...
def bench_variable_block_sparse_attention(
    ...
    q = torch.randn(1, num_qo_heads, head_dim, dtype=torch.half, device="cuda")
    ...
    print(
        f"bench_variable_block_sparse_attention (num_qo_heads={num_qo_heads}, num_kv_heads={num_kv_heads}, head_dim={head_dim}, seq_len={seq_len}, "
        f"num_blocks_row={num_blocks_row}, num_blocks_col={num_blocks_col}, "
        f"block_density={block_density}), "
        f"sparse fa2-template: {sparse_ms_fa2 * 1e3:.3f} us "
        f"dense fa2-template: {dense_sm80_ms * 1e3:.3f} us "
    )
